...
void *mempool_get(size_t size)
{
    return ncx_slab_alloc(g_mempool, size);
}

void mempool_put(void *p)
{
    ncx_slab_free(g_mempool, p);
}

void mempool_status(void)
{
	ncx_slab_stat_t stat;
    ncx_slab_stat(g_mempool, &stat);
}

/*
 * 初始化存储池
 */
ncx_slab_pool_t * init_mempool(uint64_t usize)
{
    uint64_t size;

    if (usize == 0 || usize > MEMPOOL_MAX_SIZE || usize < MEMPOOL_MIN_SIZE) {
        size = MEMPOOL_MAX_SIZE;
    } else {
        size = usize;
    }

    g_mempool = (ncx_slab_pool_t *)malloc(size);
    if (g_mempool == NULL) {
        error("Error: failed to malloc for g_mempool, exit!\n");
        exit(-1);
    }

    g_mempool->addr = (void *)g_mempool;
    g_mempool->min_shift = 3;
    g_mempool->end = (u_char *)g_mempool + size;
...
ncx_slab_init(ncx_slab_pool_t *pool)
{
    ...
    if (ncx_slab_max_size == 0) {
        ...
        ncx_slab_exact_size = ncx_pagesize / (8 * sizeof(uintptr_t));//arm = 128, x86 = 64
    ...
    info("ncx_pagesize = %lu, ncx_slab_max_size = %lu, ncx_slab_exact_size = %lu, start:%p, end:%p",
            ncx_pagesize, ncx_slab_max_size, ncx_slab_exact_size, pool->start, pool->end);
}
...
ncx_slab_free_locked(ncx_slab_pool_t *pool, void *p)
{
    ...
    if ((u_char *) p < pool->start || (u_char *) p > pool->end) {
        error("ncx_slab_free(): outside of pool: %p", p);
        goto fail;
    }
